...
const stepsToShow = ladderSteps.slice(-(totalQuestions || 15));
  ...
  return (
      <>
          ...
          <div className="hidden md:block w-48 bg-slate-900/50 border border-slate-800 rounded-xl p-4 shadow-xl">
              ...
              <div className="space-y-1">
                ...
                `}
                          >
                              <span
                                  className={`text-sm font-medium ${isCurrent ? "text-amber-400" : ""}`}
                              >
                                  {step.title}
                              </span>
                              <span
                                  className={`text-xs font-mono ${isCurrent ? "text-amber-400" : ""}`}
                              >
                                  {step.salary}
                              </span>
                              {isCurrent && (
                                  <span className="absolute -right-1 text-amber-400">
                                      ▶
                                  </span>
                              )}
                          </div>
          ...
          <div className="md:hidden w-full bg-slate-900/80 border border-slate-800 rounded-xl p-3 shadow-xl mb-4">
              <div className="flex items-center justify-between gap-4">
                  {/* Left Column: Question Counter */}
                  ...
                  <div className="flex flex-col text-right">
                      ...
                      <span className="text-base font-bold text-amber-400 truncate max-w-[180px]">
                          {currentStep.title}
                      </span>
                  </div>
              </div>
              ...
              {/* Mini progress bar */}
              <div className="w-full h-2 bg-slate-800 rounded-full mt-2.5 overflow-hidden border border-slate-700/50">
                  <div
